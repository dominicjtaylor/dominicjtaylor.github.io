...
loader.load('js/textures/Terrains/Terrain001_4K.exr', (texture) => {

    const { width, height, data } = texture.image;
    console.log('Heightmap size:', width, height);

	const step = 8; // take every 8th pixel → 512x512 segments
	const wSeg = Math.floor(width / step);
	const hSeg = Math.floor(height / step);
	const geometry = new THREE.PlaneGeometry(100, 100, wSeg-1, hSeg-1);
    geometry.rotateX(-Math.PI/2);

    // Displace vertices using the red channel
    const positions = geometry.attributes.position;
for (let i = 0; i < positions.count; i++) {
    const x = (i % wSeg) * step;
    const y = Math.floor(i / wSeg) * step;
    const idx = (y * width + x) * 4; // RGBA
    const h = data[idx];
    positions.setY(i, h * 15);
}

    geometry.computeVertexNormals();

    // const material = new THREE.MeshStandardMaterial({ color: 0x556B2F, flatShading: false, side: THREE.DoubleSide});
